"""NIFTY gap-up 15-minute range breakout — exactly the 4-step rule as specified.

STEP 1 - GAP UP
    GapUp% = (TodayOpen - PDC) / PDC * 100        PDC = previous session close
    Trade the day only if GapUp% >= 0.30

STEP 2 - RANGE BREAKOUT
    ...
    CLOSE is above OR high is the breakout candle.
    Entry = OPEN of the next candle (cn+1), long.

STEP 3 - STOPLOSS
    SL = LOW of cn.  The report carries both readings of it, selectable:

      close  the rule as written - "cn low < cn+n close" - the position is
             closed only when a later candle CLOSES below cn's low, and the
             ...
             at 1R (2026-03-17 filled 47.35 points below the level, -1.73R).
      touch  a resting SL order at cn's low, filled the moment price trades
...
2. A CLOSE-CONFIRMED STOP DOES NOT CAP THE LOSS AT 1R.  Reporting "risk =
   ...
   loss: all three stops in the set filled below the level, 67 points in
   total.  Hence --stop-mode, and hence the day table shows the fill, the
   level and the realised R side by side.
...
is squared off at the square-off time (default 15:25).

Run from backend/:
    python scripts/gapup_breakout_v1.py
    python scripts/gapup_breakout_v1.py --offline
    python scripts/gapup_breakout_v1.py --gap 0.5

Output: gapup_breakout_v1_report.html (self-contained, no CDN).
"""
...
def walk(bars: list[dict], i_entry: int, entry: float, sl: float,
         target: float, square_off: str, stop_mode: str) -> dict:
    """Forward walk from the entry candle until target, stop or square-off.

    stop_mode "close": the stop is a 15-minute CLOSE below cn's low, so it can
    only fire at a bar close and the fill is that close.
    stop_mode "touch": the stop is a resting order at cn's low, checked every
...
def simulate_day(day: str, minutes: list[dict], pdc: float, *,
    ...
    if gap < gap_pct:
        row["status"] = "no gap up"
        return row
    row["gapup"] = True

    # ---- Step 2: 15-minute range breakout -------------------------------
    ...
    if risk <= 0:
        row["status"] = "entry at/below SL"
        return row

    # ---- Step 4: the same trade at every RR, under both stop rules ------
    row["status"] = "trade"
    row["met"] = True
    # gap up then a break of the 15m HIGH - always LONG,
    ...
    for mode in stop_modes:
        for r in rr_values:
            ...
            res = walk(bars, idx + 1, entry, sl, target, square_off, mode)
            pts = res["exit"] - entry
...
<title>NIFTY Gap-up Breakout __FROM__ to __TO__</title>
<style>
  :root {
    color-scheme: light;
...
<h1>NIFTY Gap-up &rarr; 15-minute Range Breakout &middot; gap &ge; __GAP__% &middot; cn __SCANF__&ndash;__SCANT__</h1>
<div class="sub">
  <b>Step 1</b> &mdash; GapUp% = (TodayOpen &minus; PDC) / PDC &times; 100, traded only when
  ...
  <b>close is above the range high</b> is the breakout, and the trade is a <b>BUY at the open of
  cn+1</b>. <b>Step 3</b> &mdash; stop is the <b>low of cn</b>, hit when a later candle closes
  below it &mdash; or, in <b>touch</b> mode, the moment price trades at that low.
...
<div class="card">
  <h2>Every stop rule &times; risk:reward &mdash; identical entries in all eight rows</h2>
  <div class="ctrl dim">The 10 entries never change; only how the trade is closed does.
    Click any row to select it.</div>
...
const stopShort = k => k === 'touch' ? 'Touch cn low' : 'Close below cn low';
...
function renderDays() {
  ...
  const body = rows.map(r => {
    ...
    return `<tr><td>${r.date}</td><td class="pos">yes</td>
      ...
      <td class="num pos">${n2(r.gap_pct)}</td><td class="num dim">${n2(r.or_high)}</td>
...
function renderAll() {
  ...
  document.getElementById('rrDesc').innerHTML =
    `Stop: ${stopLabel(curS)}. Target = Entry + (Entry &minus; low of cn) &times; `
