"""NIFTY opening-range reversal — v1, the strict entry rule.

Self-contained: this file is the whole v1 strategy.  It imports nothing from
the other version files, so v1 keeps working no matter what happens to them.

THE RULE
--------
Opening range = the first 5-minute candle (09:15 -> 09:20), built from the five
1-minute candles 09:15..09:19.  OR_high = max(high), OR_low = min(low).

At the decision time `t` NIFTY must sit OUTSIDE that range, must never have
crossed the opposite edge since 09:20, and 09:20->t must be a clean trend that
way.  We fade it:

    below OR_low  + down-trend  ->  expect a reversal UP    ->  BUY ATM CE
    above OR_high + up-trend    ->  expect a reversal DOWN  ->  BUY ATM PE

No confirmation filters - those arrive in v3.
...
Both legs are option PURCHASES, so PnL is always (exit price - entry price).

Run from backend/:
    python scripts/reversal_v1.py --offline
    python scripts/reversal_v1.py --side ce --from 2026-01-01 --to 2026-09-03

Output: reversal_v1_report.html (self-contained, no CDN).
"""
...
REPORT_HTML = os.path.join(REPORTS_DIR, "reversal_v1_report.html")
...
BASE_RULES = {"v1": detect_signal_v1}
...
RULE_HTML = (
    "<b>v1 &mdash; strict rule.</b> Opening range = first 5-min candle "
    "(09:15&ndash;09:20). At <b>t</b>, NIFTY must be <b>outside</b> that range, must "
    "never have crossed the opposite edge since 09:20, and 09:20&rarr;t must be a clean "
    "trend. We fade it: down &rarr; <b>BUY ATM CE</b>, up &rarr; <b>BUY ATM PE</b>. "
    "No confirmation filters. Trend mode: <b>__TREND__</b>; nearest expiry; "
    "stop <b>__STOPPCT__%</b> of spot."
)
...
def main() -> None:
    ...
    asyncio.run(run(
        ...
        detect_fn=make_detector("v1", side=args.side),
