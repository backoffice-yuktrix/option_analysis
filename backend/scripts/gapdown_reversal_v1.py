...
1. PDC MUST COME FROM THE DAILY CANDLES.  The 1-minute feed's last candle is
   ...
   94 of 124 sessions, by up to 85 NIFTY points.  Using it for PDC moves
   GapDown% enough to mis-classify days on both sides of the 0.30% line.
   load_daily() is the only source of PDC.
...
REPORT_HTML = os.path.join(SCRIPT_DIR, "gapdown_reversal_v1_report.html")
...
async def load_daily(offline: bool, from_date: date, to_date: date) -> dict[str, float]:
    ...
    GapDown% and mis-classifies days on both sides of the 0.30% line.
    """
