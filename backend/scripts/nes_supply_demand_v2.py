"""NES supply/demand v2 - v1's rule, with the two entry filters that survived.
...
Run from backend/:
    python scripts/nes_supply_demand_v2.py --offline
    python scripts/nes_supply_demand_v2.py --tf 1 --target liq
    python scripts/nes_supply_demand_v2.py --eod hold --buffer 2

Output: ../reports/nes_supply_demand_v2_report.html (self-contained, no CDN).
"""
...
REPORT_HTML = os.path.join(REPORTS_DIR, "nes_supply_demand_v2_report.html")
...
MIN_SWEEP_DEPTH = 0.1          # sweep must pierce this many zone widths
...
def run_state_machine(bars: list[dict], *, pivot: int, minor_pivot: int,
    ...
    def filter_reason(b: dict, sig: dict) -> str | None:
        """v2's two entry filters.  Returns why the signal is rejected, or None.

        Both are evaluated on information available at the entry candle's
        close, so neither can look forward."""
        if b["start"] >= entry_until:
            return (f"entry at {b['start']} is at or after the {entry_until} "
                    f"cut-off - not enough session left for a 2R move")
        width = sig["zone_high"] - sig["zone_low"]
        depth = abs(sig["sweep_px"] - sig["sweep_level"]) / max(width, 1e-9)
        if depth < min_sweep_depth:
            return (f"sweep pierced the swing by only {depth:.2f} zone widths "
                    f"(needs {min_sweep_depth:.2f}) - a brush, not a stop-hunt")
        return None
...
<div class="sub">
  ...
  supply zone's high &mdash; the setup is dead and the zone <b>flips in place</b>: the same
  low/high becomes the opposite zone, that direction is blocked, and the unlock level is
  frozen at the session extreme as it stood <b>before</b> the failure candle. The reversal
  then needs a retest of the flipped zone and the same three-step confirmation the other way.
  One primary and one reversal per day, so at most <b>two</b> trades.<br>
...
def main() -> None:
    ...
    ap.add_argument("--min-sweep-depth", type=float, default=MIN_SWEEP_DEPTH,
                    help="sweep must pierce the swing by this many zone widths "
                         "(v2 filter 2)")
