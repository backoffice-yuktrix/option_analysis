"""NES supply/demand v3 - the zone alone.  No sweep, no micro-BOS.
...
Run from backend/:
    python scripts/nes_supply_demand_v3.py --offline
    python scripts/nes_supply_demand_v3.py --tf 1 --target liq
    python scripts/nes_supply_demand_v3.py --eod hold --buffer 2

Output: ../reports/nes_supply_demand_v3_report.html (self-contained, no CDN).
"""
...
REPORT_HTML = os.path.join(REPORTS_DIR, "nes_supply_demand_v3_report.html")
...
def run_state_machine(bars: list[dict], *, pivot: int, minor_pivot: int,
    ...
    def make_signal(i: int, b: dict, kind: str, side: str) -> dict:
        """Entry is the close of the candle that TOUCHED the zone; the stop is
        the far edge of that same zone, pushed out by stop_pad zone widths."""
        entry = b["close"]
        width = zone_hi - zone_lo
        if side == "LONG":
            sl = zone_lo - stop_pad * width - buffer
        else:
            sl = zone_hi + stop_pad * width + buffer
    ...
    for i, b in enumerate(bars):
        ...
        if state == "WAIT_FIRST_BOS":
            ...
            if zone_failed(b):
                ...
                if not allowed:
                    add(t, "BLOCKED", f"{side} is locked - no entry")
                    ...
                    state = "WAIT_ZONE_FAIL" if primary_taken else "WAIT_PRIMARY_RETEST"
            ...
            if reversal_taken:
                ...
                side = "LONG" if zone_type == "DEMAND" else "SHORT"
                sig = make_signal(i, b, "reversal", side)
                allowed = long_allowed if side == "LONG" else short_allowed
                add(t, "RETEST", f"Retested the flipped {zone_type.lower()} zone "
                    f"({zone_lo:.2f} - {zone_hi:.2f})")
                if allowed and filter_reason(b, sig) is None:
                    out["signals"].append(sig)
                    out["status"] = "trade"
                    reversal_taken = True
                state = "DONE"
