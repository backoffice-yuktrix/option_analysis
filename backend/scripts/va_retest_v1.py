"""Value-area break and retest - the band breaks, the retest holds, trade on.

THE RULE, AS SPECIFIED
----------------------
  1  Build a fixed-range volume profile (POC, VAH, VAL).
  2  On 5-minute candles, profile the PREVIOUS day in full.  Extend the three
     lines into the current day.
  3A Do not care where the market opened, or whether the previous day closed
     inside or outside the value area.
  3B If the candles break the VAH, after n 5-minute candles (n = 4,5,6,7)
     retest the VAH, see the confirmation, then enter LONG.
     SL 25 points below the VAH.  Target = risk x 2.
  3C If the candles break the VAL, after n candles retest the VAL, see the
     confirmation, then enter SHORT.
     SL 25 points [above the VAL].  Target = risk x 2.

This is the third of three value-area rules the user specified, and the only one
that made money.  Run side by side on the same profile, the same exits and the
same pricing over 245 sessions:

    S1  POC reversal        36 trades   avg R -0.179   -2.86% premium
    S2  break and RECLAIM   60 trades   avg R -0.079   -2.09% premium
    S3  break and RETEST   109 trades   avg R +0.239   +3.42% premium

S2 AND S3 ARE OPPOSITE BETS ON THE SAME EVENT and they agree.  S2 fades a band
break and S3 trades its continuation; on 3-minute candles S2 is negative at
t = -2.57, which inverted is +2.57.  Two rules, one conclusion: a value-area
break HOLDS more often than it FAILS.  That is the finding, and this file is the
side of it that pays.


WHAT THE SPEC LEFT OPEN, AND WHAT WAS CHOSEN
---------------------------------------------
1. WHAT "CONFIRMATION" MEANS.  Chosen by the user: the retest candle must REACH
   the band and still CLOSE beyond it - the level held.  A touch with no close
   test would enter without evidence the band did anything, and a separate
   confirming candle would enter later still.  --confirm touch and
   --confirm two-candle keep both alternatives one flag away.

2. WHEN THE RETEST COUNTS.  "after (n) 5 min candles (n = 4,5,6,7)" is read as a
   WINDOW: the retest must land between 4 and 7 candles after the breaking
   candle.  Earlier is not a retest, it is the break still printing; later is a
   different event.  --wait-min / --wait-max move it.

3. STEP 3C PUTS THE SHORT'S STOP ON THE WRONG LEVEL.  It says "place the SL
   above the VAH" on a trade taken at the VAL.  The VAH is the far side of the
   value area - typically 80 to 120 points away from a VAL short - so the
   literal reading is not a wide stop, it is not a stop at all.  It is the same
   copy-paste family as strategy 1's step 3B.  Mirrored to VAL + 25, the only
   reading that puts the stop on the losing side.  --literal-3c reproduces the
   defect and reports what it costs.

4. WHICH DAY'S PROFILE.  Step 2 says the previous day, on 5-minute candles, so
   the levels are fixed before the session starts and nothing about them is
   known late.  3 and 5 minute signal timeframes are both computed; 15-minute
   was dropped earlier for having too few trades and unstable halves.

5. HOW MANY TRADES A DAY.  One, the first placeable retest - the user's rule.
   --mode flow re-arms after each trade closes and is computed alongside.

ENTRY is the OPEN OF THE NEXT CANDLE after the confirming close.  The
confirmation is a close, so it is not known until the candle is over and that
candle cannot be traded at its own open - see pd_zone_reversal, where exactly
that substitution was worth the entire result.

EXITS always walk on 1-MINUTE candles whatever the signal timeframe: a stop is a
price that can trade at any moment inside a bar, and resolving it only at
5-minute closes would flatter every result.  Within a minute the order is
square-off, then stop, then target, so a minute spanning both is a loss.


WHAT THE NUMBERS CAN AND CANNOT SHOW
-------------------------------------
Per-trade R has a standard deviation near 1.37, so a book of n trades can only
distinguish an edge of about 2.7/sqrt(n) from zero.  At 109 trades a year that
is +-0.26 R, and this rule measured +0.239 - right at the edge, t = +1.76.

Taking BOTH timeframes raises it to 232 trades and t = +2.09, but that is
overstated: 109 of the 123 trade days appear in both, so the same day is counted
twice.  Clustered by day the honest figure is 123 independent days, avg R
+0.196, t = +1.69, 95% CI [-0.032, +0.424] - NOT significant, though closer than
anything else in this study got.  The report prints t and the interval beside
every mean for that reason.
"""
...
REPORT_HTML = os.path.join(REPORTS_DIR, "va_retest_v1_report.html")
TRADES_CSV = os.path.join(RESULTS_DIR, "va_retest_v1_trades.csv")
